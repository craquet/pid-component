...
import { DOI_examples, ROR_examples, URL_examples } from '../../../../../../examples';
...
describe('pid-component', () => {
  ...
  it('renderers prop is set correctly via attribute', async () => {
    const { root } = await render(
      <pid-component value={DOI_examples.DATACITE_SLIDES} renderers='["DOIType","ORCIDType"]'></pid-component>,
    );
  ...
  it('does not crash with invalid renderers JSON', async () => {
    const { root } = await render(
      <pid-component value={DOI_examples.DATACITE_SLIDES} renderers="not valid json"></pid-component>,
    );
    expect(root).toBeTruthy();
  ...
  it('renders error state since indexedDB is not available in mock-doc', async () => {
    ...
    expect(shadowHtml).toContain(URL_examples.KIT_WEBSITE);
  });
  ...
  it('fallbackToAll can be set to false programmatically', async () => {
    const { root, waitForChanges } = await render(
      <pid-component value={ROR_examples.VALID}></pid-component>,
    );
  ...
  it('darkMode can be set to system', async () => {
    const { root } = await render(<pid-component value={DOI_examples.DATACITE_SLIDES}
                                                 dark-mode="system"></pid-component>);
  ...
  it('emphasizeComponent can be set to false programmatically', async () => {
    const { root, waitForChanges } = await render(
      <pid-component value={DOI_examples.DATACITE_SLIDES}></pid-component>,
    );
    // In Stencil mock-doc, boolean false props passed via JSX attribute are not applied.
    // Set programmatically instead.
  ...
  it('component handles indexedDB unavailability gracefully', async () => {
    ...
    const shadowHtml = root.shadowRoot.innerHTML;
    expect(shadowHtml).toContain('role="status"');
    expect(shadowHtml).toContain(DOI_examples.DATACITE_SLIDES);
  });

  it('width and height props pass through', async () => {
    const { root } = await render(
      <pid-component value={URL_examples.KIT_WEBSITE} width="600px" height="400px"></pid-component>,
    );
